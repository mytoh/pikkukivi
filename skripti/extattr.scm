":"; exec gosh -- $0 "$@"
                                        ; works on only freebsd
 (use gauche.process)
 (use gauche.parseopt)
 (use kirjasto) ; colour-string

 (cond
  ((eq? (get-os-type) 'freebsd)
   (define (write-attr args)
     (let ((attr-name-space (car args))
           (attr-name (cadr args))
           (attr-value (caddr args))
           (file-name (cadddr args)))
       (run-process `(setextattr ,attr-name-space ,attr-name ,attr-value ,file-name) :wait #true)
       (print file-name)
       (print (string-append (colour-string 89 attr-name-space) "." (colour-string 30 attr-name)
                             " -> "
                             (colour-string 60 attr-value)))))

   (define (print-attr args)
     (let* ((attr-name-space (car args))
            (attr-name (cadr args))
            (file-name (caddr args))
            (str (process-output->string `(getextattr ,attr-name-space ,attr-name ,file-name)))
            (out (string-split str #\space))
            )
       (print (car out))
       (print (string-append (colour-string 89 attr-name-space) "." (colour-string 30 attr-name)
                             " -> "
                             (colour-string 60 (cadr out))))))

   (define (delete-attr args)
     (let ((attr-name-space (car args))
           (attr-name (cadr args))
           (file-name (caddr args)))
       (run-process `(rmextattr ,attr-name-space ,attr-name ,file-name) :wait #true)
       (print file-name)
       (display "removed ")
       (print (string-append (colour-string 38 attr-name-space) "." (colour-string 30 attr-name)))))

   (define (list-attr args)
     (let* ((attr-name-space (car args))
            (file-name (cadr args))
            (str (process-output->string `(lsextattr ,attr-name-space ,file-name)))
            (out (string-split str #\space))
            (file (car out))
            (attributes (cdr out))
            )
       (print file)
       (for-each
           (lambda (a)
             (display (string-append (colour-string 30 a)))
             (display " "))
         attributes)
       (newline)))
   )

  ((eq? (get-os-type) 'darwin)
   (define (print-attr args)
     (let* ((attr-name (car args))
            (file-name (cadr args))
            (str (process-output->string `(xattr -p ,attr-name ,file-name))))
       (print file-name)
       (print (string-append (colour-string 30 attr-name)
                " -> "
                (colour-string 60 str)))))

   (define (write-attr args)
     (let ((attr-name (car args))
           (attr-value (cadr args))
           (file-name (caddr args)))
       (run-process `(xattr -w ,attr-name ,attr-value ,file-name) :wait #true)
       (print file-name)
       (print (string-append (colour-string 30 attr-name)
                " -> "
                (colour-string 60 attr-value)))))

   (define (delete-attr args)
     (let ((attr-name (car args))
           (file-name (cadr args)))
       (run-process `(xattr -d  ,attr-name ,file-name) :wait #true)
       (print file-name)
       (display "removed ")
       (print (colour-string 30 attr-name))))

   (define (list-attr args)
     (let* ((file-name (car args))
            (str (process-output->string-list `(xattr ,file-name)))
            (resource (car str))
            (attributes (cdr str))
            )
       (print resource)
       (for-each
           (lambda (a)
             (display (string-append (colour-string 30 a)))
             (display " "))
         attributes)
       (newline)))
   )
  )

(define (main args)
  (let-args (cdr args)
            ((w "w|write")
             (p "p|print")
             (d "d|delete")
             . restargs)
            (cond
              (w (write-attr restargs))
              (p (print-attr restargs))
              (d (delete-attr restargs))
              (else (list-attr restargs)))))
